...
void MOTT_Net_Task(void) {
    if (Now_NetDevParameter.CheckTCPLinkFlag == true) { // 检查当前设备 TCP 是否连接
        if (!TCP_Link_OnlineFlga()) {                // 检查当前设备 TCP 是否连接
    ...
    if ((Now_NetDevParameter.NowNetOnlineFlag == false) || (Now_NetDevParameter.NowTCPLinkFlag == false)) {
        ...
        if (Now_NetDevParameter.MQTT_NET_Receive_checkTime == 0) { // 20ms check 一次，后获取数据
            ...
            if (copyDataForUART()) { // 从 UART0Ddata 获取数据
                char * AddrStart = NULL;
                // if ((AddrStart = myStrstr(Now_NetDevParameter.NetDataBuff, "{\"gw\":", ARR_SIZE(Now_NetDevParameter.NetDataBuff))) == NULL) {
                //     ClearNetDataBuff();
                //     return;
                // }
                // if (NULL != strstr(AddrStart, "}}")) {
                //     int ArrayMaxLen = ARR_SIZE(Now_NetDevParameter.NetDataBuff);
                //     memcpy(Now_NetDevParameter.NetDataBuff, AddrStart, ((Now_NetDevParameter.NetDataBuff + ArrayMaxLen) - AddrStart));
                //     Now_NetDevParameter.NetDataBuff_NowLen = strlen(Now_NetDevParameter.NetDataBuff);
                //     printf("mqtt Net Data\r\n%s\r\n", Now_NetDevParameter.NetDataBuff); // 从网口收到的 mqtt 数据                    
                //     MQTT_JSON_Analysis(Now_NetDevParameter.NetDataBuff);// 解析收到的数据，找到 josn 数据
                // }
                if (NULL == (AddrStart = myStrstr(Now_NetDevParameter.NetDataBuff, "{\"gw\"", Now_NetDevParameter.NetDataBuff_NowLen))) {
                    ClearNetDataBuff();
                    return;
                }
                if (NULL == myStrstr(AddrStart, "}}", Now_NetDevParameter.NetDataBuff - (AddrStart - Now_NetDevParameter.NetDataBuff_NowLen))) {
                    return;
                }
                printf("mqtt Net Data\r\n%s\r\n", Now_NetDevParameter.NetDataBuff); // 从网口收到的 mqtt 数据                    
...
bool copyDataForUART(void) {
    if (UART0Ddata.RxLen != 0) {
        ...
        if (Now_NetDevParameter.NetDataBuff_NowLen + UART0Ddata.RxLen <= ARR_SIZE(Now_NetDevParameter.NetDataBuff)) { // 追加到 Now_NetDevParameter.NetDataBuff
            ...
            memcpy(Now_NetDevParameter.NetDataBuff, (char *)UART0Ddata.RxBuf, UART0Ddata.RxLen); // 接收数据
            Now_NetDevParameter.NetDataBuff_NowLen = UART0Ddata.RxLen;
        }
        UART0Ddata.RxLen = 0;
